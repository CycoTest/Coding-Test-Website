...
class ValidPS {
	public String openClose(String braket) {
		Stack<Character> st = new Stack<Character>();

		for (int i = 0; i < braket.length(); i++) {
			char each = braket.charAt(i);
...
public class Main {
	public static void main(String[] args) throws IOException {
		...
		ValidPS truth = new ValidPS();
		for (int i = 0; i < N; i++) {
			tok = new StringTokenizer(br.readLine());
			String valid = tok.nextToken();

			sb.append(truth.openClose(valid)).append("\n");
		}
...
/*
 * Q: 괄호 문자열(Parenthesis String, PS)은 
